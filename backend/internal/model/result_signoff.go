...
type ResultSignoff struct {
	...
	// SpecimenRiskLevel snapshots the risk of the linked specimen captured by the
	// first reviewer; FirstReview* record the conclusion that opens 待二级复核.
	SpecimenRiskLevel string     `json:"specimenRiskLevel" gorm:"size:32"`
	FirstReviewedBy   string     `json:"firstReviewedBy" gorm:"size:80;index"`
	FirstReviewReason string     `json:"firstReviewReason" gorm:"size:500"`
	FirstReviewedAt   *time.Time `json:"firstReviewedAt,omitempty"`
	// LinkedSpecimenRiskLevel and LinkedSpecimenMissing are read-only views of
	// the currently linked specimen, enriched on read and never persisted.
	LinkedSpecimenRiskLevel string                  `json:"linkedSpecimenRiskLevel,omitempty" gorm:"-"`
	LinkedSpecimenMissing   bool                    `json:"linkedSpecimenMissing" gorm:"-"`
