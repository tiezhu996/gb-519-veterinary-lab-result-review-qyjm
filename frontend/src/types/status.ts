...
export type SignoffState = 'draft' | 'peer_review' | 'second_review' | 'signed' | 'rejected';
export const ALL_SIGNOFF_STATE: readonly SignoffState[] = ['draft', 'peer_review', 'second_review', 'signed', 'rejected'];
...
export const ENTITY_CONFIGS: readonly EntityConfig[] = [
  ...
  // peer_review branches by linked-specimen risk: high/critical -> second_review
  // (handled dynamically in EntityPage), ordinary risk -> signed.
  { key: 'resultSignoff', path: 'signoff', label: '结果签发', statuses: ['draft', 'peer_review', 'second_review', 'signed', 'rejected'] as const, primaryTransitions: { draft: 'peer_review', peer_review: 'signed', second_review: 'signed' } }
];
